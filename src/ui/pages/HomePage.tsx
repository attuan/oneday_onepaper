    ...
    <>
      ...
      {reads.length > 0 ? (
        ...
        <div className="card hero">
          ...
          <button className="btn" onClick={() => go({ name: "papers" })}>論文を追加する</button>
        </div>
      )}
      <div className="row">
        <div className="card" style={{ flex: 1 }}>
          <div className="muted">キュー</div>
          <p className="title">{q.length} 本</p>
        </div>
        <div className="card" style={{ flex: 1 }}>
          <div className="muted">読了</div>
          <p className="title">{state.papers.filter((p) => p.status === "read").length} 本</p>
        </div>
        <div className="card" style={{ flex: 1 }}>
          <div className="muted">連続記録</div>
          <p className="title">{currentStreak(state)} 日</p>
        </div>
      </div>
    </>
  );
