...
アプリの操作の説明を `src/content/guide/*.md` に置き、講座と同じレンダラで「使い方」の画面に出す(ファイル名順)。
講座は論文の読み方、使い方はアプリの操作、と分ける。論文リストが空のホームから「使い方」へ案内する。
画面の文言や機能を変えたら、ここの文章も合わせて直すこと。

### 9.2 スマホの入口(2026-09-18)

敵(ショート動画)はスマホにいるので、スマホから 2 タップで入れるようにする。ブラウザ版だけの話。

- **ホーム画面に追加**: `public/manifest.webmanifest`(アイコン、`display: standalone`)。Service Worker は置かない
  (オフラインで動かす必要が薄く、古い版がキャッシュに残る事故のほうが怖い)。
- **共有から記事を追加**: ページを `?url=&title=&text=` 付きで開くと、記事の追加欄を埋めて出す(8.0)。URL が `text` に入っていても拾う。
  - Android: ホーム画面に追加すると、共有先に出る(manifest の `share_target`)。
  - iOS / iPadOS: Web Share Target が無いので、共有シートに出るショートカットを 1 つ作ってもらう
    (「共有シートに表示」をオン → 「URL を開く」で `<ブラウザ版の URL>?url=<ショートカットの入力>`)。
- **今日の 1 本を選べる**: ホームに、キューの 2・3 番目を「こっちでも」として出す。選ぶとキューの先頭に来る。スキップと違い、回数は数えない。

### 9.1 記録を資産にする(2026-09-18)

溜めた記録が卒論や輪講に効くと分かれば、続ける理由になる。どれも `memos/` と `papers.json` から作る(`core/records.ts`)。

- **ヒートマップ**(ホーム): 直近 18 週。その日に届いた一番上の段階で濃さを変える。
- **読み返し**(ホーム): 読了から 7 日後と 30 日後(それぞれ 3 日の幅)に「思い出せますか?」を出す。先に頭の中で思い出してから、
  そのとき自分が書いた最初の 1 行を見る。「覚えていた / 忘れていた」は `meta` の `reviewed:<日数>:<論文 ID>` に残す。
- **月のまとめ**(カレンダー): その月に読んだものとメモを 1 つの Markdown に書き出す(書かなかった見出しは落とす)。
  「関連研究」の下書きは LLM に頼む(task `digest`)。材料はメモだけで、メモに無いことを足させない。
  API が使えないときは頼む文面をコピーして、好きな AI に貼ってもらう。
