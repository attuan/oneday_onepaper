...
対象：研究室に配属されたばかりの B4〜M1 で、読まなきゃと思いながらショート動画を見てしまう人(作者自身)

解決したい課題：論文を読む習慣がなく、始められない(論文を探すのが面倒、論文を読むのが面倒、読んだ論文を記録するのが面倒)

ソリューション：1日1本、読むべき論文を提示し、メモを取らせる。(すぐにAIの要約は示さず、自分で読んでメモを取らせ、血肉にさせる)メモを取ったら、AIによる要約と、メモの採点をあげ、カレンダーに記録。(リンクや書誌情報も記録)
時間がない日は 1 行だけ書けば読了(Lv1)。読了は研究室の Slack に自動で投稿できる。
...
### LLM の使い方(API キーが無くても使える)

| 道 | 要るもの | できること |
|---|---|---|
| Anthropic API | API キー | すべて |
| OpenAI 互換(OpenAI / Gemini / OpenRouter / Groq / LM Studio など) | 接続先の URL とキー(Gemini は無料枠あり。ローカルはキー不要) | すべて |
| Ollama | 手元で動く Ollama | すべて |
| Apple Intelligence(ショートカット経由) | iPadOS / iOS / macOS 26 以降の対応端末とブラウザ版 | 要約と採点 |
| 好きな AI に貼り付ける | ChatGPT や Claude などのチャット | 要約と採点 |

API の設定は 設定 → LLM。キーが無いときは、メモの画面の「AI 要約と採点」が自動でキー不要の道になる(「実行方法」で変えられる)。

**Apple Intelligence を使う準備**(初回だけ)。ショートカット App で、次の 3 アクションを並べたショートカットを `OneDayOnePaper` という名前で作る:

1. 「クリップボードを取得」
2. 「モデルを使用」— モデルは Private Cloud Compute(おすすめ)かオンデバイス。プロンプトに 1 の「クリップボード」を入れる
3. 「クリップボードにコピー」— 2 の「応答」を入れる

使うときは、ブラウザ版を Safari で開き、メモの画面で「1. Apple Intelligence で実行」→(ショートカットが動いて戻ってくる)→「2. 結果を貼り付ける」。
**2026-09-18 時点で実機(iPad)では未検証。** 画面の流れと URL の組み立ては Chromium で確かめてある。

### スマホで使う

ブラウザ版を開いて「ホーム画面に追加」するとアプリのように開ける。読みたい記事は共有から入れられる:

- Android(Chrome): ホーム画面に追加すると、共有先に「1日1本」が出る
- iPhone / iPad: ショートカット App で、「共有シートに表示」をオンにしたショートカットを作り、アクションは「URL を開く」1 つだけにして、
  URL を `https://<ユーザー名>.github.io/<リポジトリ名>/?url=<ショートカットの入力>` にする。共有シートからそれを選ぶと、記事の追加欄が埋まった状態で開く

### 読了を Slack に共有する

設定 →「読了を Slack に共有」。読了したとき、論文のタイトル・連続記録・メモの最初の 1 行を研究室などのチャンネルに投稿する。
Webhook URL は通知のものとは別に入れる(通知の「まだ読んでいません」はこちらには流れない)。メモ全体は送らない。

### BibTeX

設定の「詳しい設定と機能を出す」をオンにすると出る(CSV の取り込み、検索ソースの選択、API 使用量、ポモドーロも同じ)。
...
src/core/     本体ロジック(Tauri 非依存。テスト可能)
  ...
  share.ts    読了を Slack に共有するときの文面
  pomodoro.ts ポモドーロの状態遷移
  markdown.ts 講座用の Markdown レンダラ
  llm/        プロバイダ抽象層(Anthropic / Ollama / OpenAI 互換)・要約・採点・API を通さない道(handoff.ts)
  usage/      単価表・概算
  store/      データの保存
...
- v3: 読了の段階(Lv1 ひとこと 20 字 / Lv2 要点 80 字 / Lv3 しっかり 200 字。Lv1 で連続記録が続く)、読了を Slack に共有、
